...
#for exam
mv $taskPath/tmp/RAM_tb_exam_$1_Task$2.vhdl $descPath/RAM_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/RAM.vhdl $descPath/RAM_beh.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/RAM_tb_exam.vhdl"
    fi
fi
