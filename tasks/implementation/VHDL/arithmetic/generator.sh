...
########################################################################
# generator.sh for VHDL task arithmetic
...
mv ${task_path}/tmp/arithmetic_${user_id}_Task${task_nr}.vhdl ${desc_path}/arithmetic.vhdl

#copy static files to user's description folder
cp ${task_path}/static/arithmetic_beh.vhdl ${desc_path}

#exam
mv ${task_path}/tmp/arithmetic_tb_exam_${user_id}_Task${task_nr}.vhdl ${desc_path}/arithmetic_tb_exam.vhdl
...
##########################
task_attachments=""
task_attachments_base="${desc_path}/desc_${user_id}_Task${task_nr}.pdf ${desc_path}/arithmetic.vhdl ${desc_path}/arithmetic_beh.vhdl"

if [ -n "${mode}" ];
then
    if [ "${mode}" = "exam" ];
    then
        task_attachments="${task_attachments_base} ${desc_path}/arithmetic_tb_exam.vhdl"
    fi
fi
