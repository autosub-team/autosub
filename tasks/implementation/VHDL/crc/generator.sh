...
########################################################################
# generator.sh for VHDL task crc
...
cp ${task_path}/static/crc_beh.vhdl ${desc_path}
cp ${task_path}/static/fsr_beh.vhdl ${desc_path}

#move generated files to user's description folder
mv ${task_path}/tmp/crc_${user_id}_Task${task_nr}.vhdl ${desc_path}/crc.vhdl
mv ${task_path}/tmp/fsr_${user_id}_Task${task_nr}.vhdl ${desc_path}/fsr.vhdl

#for exam
mv ${task_path}/tmp/crc_tb_exam_${user_id}_Task${task_nr}.vhdl ${desc_path}/crc_tb_exam.vhdl
...
##########################
task_attachments=""
task_attachments_base="${desc_path}/desc_${user_id}_Task${task_nr}.pdf ${desc_path}/fsr_beh.vhdl ${desc_path}/crc_beh.vhdl ${desc_path}/fsr.vhdl ${desc_path}/crc.vhdl"

if [ -n "${mode}" ];
then
    if [ "${mode}" = "exam" ];
    then
        task_attachments="${task_attachments_base} ${desc_path}/crc_tb_exam.vhdl"
    fi
fi
