...
mv ${task_path}/tmp/ROM_${user_id}_Task${task_nr}.vhdl ${desc_path}/ROM.vhdl

#copy static files to user's description folder
cp ${task_path}/static/ROM_beh.vhdl ${desc_path}

#for exam
mv ${task_path}/tmp/ROM_tb_exam_${user_id}_Task${task_nr}.vhdl ${desc_path}/ROM_tb_exam.vhdl
...
##########################
task_attachments=""
task_attachments_base="${desc_path}/desc_${user_id}_Task${task_nr}.pdf ${desc_path}/ROM.vhdl ${desc_path}/ROM_beh.vhdl"

if [ -n "${mode}" ];
then
    if [ "${mode}" = "exam" ];
    then
        task_attachments="$task_attachments_base ${desc_path}/ROM_tb_exam.vhdl"
    fi
fi
