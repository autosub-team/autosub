...
########################################################################
# generator.sh for VHDL task truth_table
...
cp ${task_path}/static/truth_table.vhdl ${desc_path}
cp ${task_path}/static/truth_table_beh.vhdl ${desc_path}

#for exam
cp ${task_path}/exam/testbench_exam.vhdl ${desc_path}/truth_table_tb_exam.vhdl
...
##########################
task_attachments=""
task_attachments_base="${desc_path}/desc_${user_id}_Task${task_nr}.pdf ${desc_path}/truth_table.vhdl ${desc_path}/truth_table_beh.vhdl"

if [ -n "${mode}" ];
then
    if [ "${mode}" = "exam" ];
    then
        task_attachments="$task_attachments_base ${desc_path}/truth_table_tb_exam.vhdl"
    fi
fi
