...
######################################################################################
# generator.sh for VHDL task SC_CU
...
if [ "$RET" -ne "$zero" ];
then
    echo "ERROR with pdf generation for Task$2 !!!" >&2
fi
...
cp $taskPath/static/SC_CU.vhdl $descPath
cp $taskPath/static/SC_CU_beh.vhdl $descPath

#for exam
#cp $taskPath/exam/testbench_exam.vhdl $descPath/SC_CU_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/SC_CU.vhdl $descPath/SC_CU_beh.vhdl"

# if [ -n "$4" ];
# then
#     if [ "$4" = "exam" ];
#     then
#         taskAttachments="$taskAttachmentsBase $descPath/SC_CU_tb_exam.vhdl"
#     fi
# fi
