...
cp $taskPath/static/crc_beh.vhdl $descPath
cp $taskPath/static/fsr_beh.vhdl $descPath

#move generated files to user's description folder
mv $taskPath/tmp/crc_$1_Task$2.vhdl $descPath/crc.vhdl
mv $taskPath/tmp/fsr_$1_Task$2.vhdl $descPath/fsr.vhdl

#for exam
mv $taskPath/tmp/crc_tb_exam_$1_Task$2.vhdl $descPath/crc_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/fsr_beh.vhdl $descPath/crc_beh.vhdl $descPath/fsr.vhdl $descPath/crc.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/crc_tb_exam.vhdl"
    fi
fi
