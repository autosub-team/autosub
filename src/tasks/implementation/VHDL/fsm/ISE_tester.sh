...
python3 scripts/generateTestBench.py "$3" > $userTaskPath/fsm_tb_$1_Task$2.vhdl
...
#copy used testbench
cp $userTaskPath/fsm_tb_$1_Task$2.vhdl $userTaskPath/used_tbs/fsm_tb_$1_Task$2_Submission${submissionNrLast}.vhdl

#--------------------------------------------------#

#copy the entity vhdl file for testing to user's folder
cp $descPath/fsm.vhdl $userTaskPath
#copy the package vhdl file for testing to user's folder
...
#package, not from user, should have no errors
vhpcomp fsm_pkg.vhdl
RET=$?
if [ "$RET" -ne "$zero" ]
then
...
vhpcomp fsm_beh.vhdl 2> /tmp/$USER/tmp_Task$2_User$1
RET=$?
...
then
    ...
    #cat $userTaskPath/isim.log | xargs sed 's/\\n/\n/g'  >>$userTaskPath/error_msg
