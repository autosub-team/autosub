...
########################################################################
# generator.sh for VHDL task fsm
...
cp $taskPath/static/fsm_beh.vhdl $descPath
cp $taskPath/static/fsm_pkg.vhdl $descPath

#for exam
cp $taskPath/exam/testbench_exam.vhdl $descPath/fsm_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/fsm_pkg.vhdl $descPath/fsm.vhdl $descPath/fsm_beh.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/fsm_tb_exam.vhdl"
    fi
fi
