...
########################################################################
# generator.sh for VHDL task example 
...
cp $taskPath/static/ALU.vhdl $descPath
cp $taskPath/static/ALU_beh.vhdl $descPath

#for exam
cp $taskPath/exam/testbench_exam.vhdl $descPath/ALU_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/ALU.vhdl $descPath/ALU_beh.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/ALU_tb_exam.vhdl"
    fi
fi
