...
cp $taskPath/static/truth_table.vhdl $descPath
cp $taskPath/static/truth_table_beh.vhdl $descPath

#for exam
cp $taskPath/exam/testbench_exam.vhdl $descPath/truth_table_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/truth_table.vhdl $descPath/truth_table_beh.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/truth_table_tb_exam.vhdl"
    fi
fi
