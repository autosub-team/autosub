...
mv $taskPath/tmp/arithmetic_$1_Task$2.vhdl $descPath/arithmetic.vhdl

#copy static files to user's description folder
cp $taskPath/static/arithmetic_beh.vhdl $descPath

#exam
mv $taskPath/tmp/arithmetic_tb_exam_$1_Task$2.vhdl $descPath/arithmetic_tb_exam.vhdl
...
##########################
taskAttachments=""
taskAttachmentsBase="$descPath/desc_$1_Task$2.pdf $descPath/arithmetic.vhdl $descPath/arithmetic_beh.vhdl"

if [ -n "$4" ];
then
    if [ "$4" = "exam" ];
    then
        taskAttachments="$taskAttachmentsBase $descPath/arithmetic_tb_exam.vhdl"
    fi
fi
