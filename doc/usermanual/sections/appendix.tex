...
allow\_requests & Decides if the system runs in request queue mode or linear queue mode. If this is activated
	auto\_advance and allow\_skipping are set to no. Possible values: once multiple no & no \\
\hline
allow\_skipping & Allows users to skip ahead to the next task with a skip E-Mail. 
	Only possible in linear task queue mode. Possible values: yes no & no \\
\hline
